...
- **Items 5/6 — Flux webhook delivery + guard — BUILT `977636e` (unpushed, awaiting say-so).**
  ...
  current. Verified end-to-end via `infra-init` render (hosts inject, webhook.yaml renders,
  Infra Build packs it). **Deferred to `srv`:** platform auto-configuring the GitHub-side webhook
  (needs the GitHub App); operator wires it by hand until then. **`./infra` not touched** —
  operator's re-scaffold picks up the new baseline.
