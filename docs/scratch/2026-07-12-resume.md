...
All slices landed, hermetic + smoke green. **Unpushed on `main`** — push waits for the
operator's say-so.

## Landed this session (unpushed)
...
- `0ad28ea` **framework: CUE module path is a scaffold input** — the big one. `Framework` gains
  `RequiredScaffoldInputs(wd)` (declares inputs to prompt, by name) + `ScaffoldData` (builds
  template data from inputs); the driver stays framework-agnostic (no CUE_MOD_PREFIX→ModulePath
  routing in `planSpecFiles`). Infra declares `CUE_MOD_PREFIX` greenfield only, validates it is a
  legal CUE module path; the 6 onboarding frameworks embed `noScaffoldInputs`. Dropped
  `Project.ImportPrefix` + the `Generate` write (was write-only). ADR rewritten+renamed to
...
- **Design B** (Scaffold returns fully-resolved files, `Resolve` becomes Infra-internal) — I took
  Design A (`ScaffoldData` method + generic `Resolve` in `planSpecFiles`). B is a clean follow-on
  if wanted; A already removes all routing knowledge from the driver.
