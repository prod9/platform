...
- **prod9-main bring-up (2026-07-16, later):** cluster lke632028 live — gateway on
  reserved IP `139.162.23.194`, HTTPS issuing via ListenerSets, flux applied but
  **suspended**. Releases v0.9.1→v0.9.4 cut same day (v0.9.x = v2 line, ADR'd;
  v0.10.0 mis-cut withdrawn). Baseline now: launcher stamps its own release,
  distributed-hosts (gateway + cluster-issuer components, ListenerSets), cert-manager
  `--enable-gateway-api{,-listenerset=true}` (live-verified flags), **provider-neutral**
  (ADR 2026-07-16; Linode annotations = infra-repo edits; `set-unless-empty` REJECTED —
  no DSL conditionals). New infra repo: `~/Documents/prod9/infra/infra-v2` (local only).
  **Still blocking:** repo-name/wipe ruling (`gh:prod9/infra` taken by dead repo), stale
  `ghcr.io/prod9/infra:latest` package deletion, first real publish → unsuspend flux;
  defs pack-level `#UseCertManager` default routed to `prod9.infra-defs.claude`.
