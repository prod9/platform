...
- **🎯 NEXT (fresh session): RESUME the srv 1-by-1 — IN PROGRESS, nothing settled.**
  ...
  *"the 1-by-1 is IN PROGRESS and only like a few items have been settled"* — the few
  ...
  close-out. Start with the recorded collision: the walk renames the concern to `install`
  (`GET /install`) while his ruling 3 names `GET /setup` in the exact set. Do **not**
  draft specs/ADRs from the record and do **not** hand it to a subagent — drafting is
  downstream of a closed walk. (Execution mode once it closes, per chakrit: write the
  changes into specs/ADRs/guides first; an unattended subagent implements from those
  later.)
