...
- **🎯 NEXT (fresh session): EXECUTE the settled srv 1-by-1.** The walk ran 2026-07-17
  and every item is decided — full record + batch tasks in
  [`2026-07-17-srv-1by1.md`](2026-07-17-srv-1by1.md) (committed). Execution mode per
  chakrit: **write the changes into specs/ADRs/guides first; an unattended subagent
  implements from those later.** So next session = draft the spec/ADR/notes updates
  from the record (routing via `docs/README.md`), then the batch tasks. Headline
  decisions: bare routes (no `/api`), manifest-setup flow deleted → manual App creation
  + env-config credentials + rolling-install model (`GET /install`, state-gated, no
  secret), flux webhook dropped → SA-based flux observability endpoint, fx-worker
  reconcile-against-engine replaces boot requeue, event-sourcing builds
  (`build_attempts`, computed status). The original rulings below are superseded by
  the record where they differ:
