...
- **Go-live handoff sent to `prod9.infra.claude` (2026-07-17,
  `/tmp/go-live-handoff-prod9.platform.md`):** all three items theirs — repo decision
  (old `gh:prod9/infra` disposable per rework Law; platform constraints: `*infra*` name
  glob, `ghcr.io/prod9/infra` wiring), stale ghcr package deletion, then publish
  (v0.9.5) → webhook → unsuspend → e2e verify. Awaiting their DONE/STUCK. Also pending
  externally: defs pack-level `#UseCertManager` default (routed to
  `prod9.infra-defs.claude`; platform.cue carries the interim mixin line).
